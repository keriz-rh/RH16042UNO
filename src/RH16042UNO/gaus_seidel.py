"""
Módulo que implementa el método iterativo de Gauss-Seidel para resolver
...
def gauss_seidel(A, b, x0=None, tol=1e-6, max_iter=100):
    ...
    for iterations in range(max_iter):
        ...
        # Calcular el nuevo x usando el método de Gauss-Seidel
        for i in range(n):
            if abs(A[i, i]) < 1e-10:
                raise ValueError(f"El elemento diagonal A[{i},{i}] es cercano a cero")
            
            # Sumatoria de los términos que involucran valores ya actualizados
            s1 = np.sum(A[i, :i] * x[:i])
            # Sumatoria de los términos que aún no se actualizan
